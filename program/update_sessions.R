library(httr2)
req <- request("https://conf.nectric.com.au/api/events/iif-osf-2025/")
...
sessions_tidy <- map_dfr(sessions$slots,recurse_tibble) |> 
  ...
  transmute(
    start = as.POSIXct(start, format = "%Y-%m-%dT%H:%M:%S+08:00"),
    duration,
...
write_session_qmd <- function(x, ...) {
  ...
  if(is.null(x$description)) {
    chat <- ellmer::chat_openai(
      system_prompt = "Briefly summarise the key session topics in a plain text from the following abstract in 2 sentences. The summary should start with a background details sentence, followed a sentence detailing the key topics of the session in passive voice.",
      model = "gpt-5.1"
    )
  ...
  x$yml <- yaml::as.yaml(
    list(
      pagetitle = paste("IIF OSF 2025:", x$title),
      date = format(x$start),
